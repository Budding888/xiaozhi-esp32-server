...
## 1.启动ragflow知识库【注意：启动知识库的时候，必须启动ollama】
## 进入D:\pythonWorkSpace\ragflow\docker容器目录下，执行: docker-compose -f docker-compose.yml up -d

或者直接进入到docker容器，直接启动下面3个容器：es、ragflow-cpu、minio
```angular2html
