...
# 2.项目虚拟环境的创建与管理
## 1.创建conda虚拟环境
conda create -n xiaozhi-esp32-server python=3.11
...
## 4.1 启动ollama（启动ragflow知识库之前先启动ollama。因为要使用 bge-m3）

### 首次运行时
### 启动ollama、然后进入ollama内部，下载chat model 和 文本嵌入（Embedding）模型
...
## 4.2 启动ragflow(es、ragflow-cpu、minio)

>  进入D:\pythonWorkSpace\ragflow\docker容器目录下
>  执行: docker-compose -f docker-compose.yml up -d

### 直接进入到docker容器，直接启动下面3个容器：es、ragflow-cpu、minio
```angular2html

# 进入你docker-compose_all.yml所在的文件夹
...
time="2026-06-08T14:08:33+08:00" level=warning msg="Found orphan containers ([docker-redis-1]) for this project. If you removed or renamed this service in your compose file, you can run this command with the --remove-orphans flag to clean it up."
[+] up 3/3
 ✔ Container docker-es01-1        Started                                                                           0.3s
 ✔ Container docker-ragflow-cpu-1 Started                                                                           0.4s
 ✔ Container docker-minio-1       Started  
```


## 2.启动Java后端服务--manager-api
```angular2html
    python app.py
```

## 3.启动Java web前端服务--manager-web
```angular2html
    cd main/manager-web/
    npm run serve
```

## 4.启动python服务端--xiaozhi-server
```angular2html
    python app.py
```
...
# 安装项目依赖过程中的问题记录（注意：如果启动时有异常，再去看下面的问题）
